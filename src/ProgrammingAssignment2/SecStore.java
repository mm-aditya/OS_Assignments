...
public class SecStore {
    ...
    private void handleRequest(Socket socketConnection) throws Exception {
        ...
        if (inLine.equals("HI")) {  // TODO: Still need to do the nonce thing.
            System.out.println(DatatypeConverter.printBase64Binary(encryptBytes("Meow".getBytes())));
            out.write(encryptBytes("Meow".getBytes()));
            out.flush();
            System.out.println("length: "+encryptBytes("Meow".getBytes()).length);

        }
        inLine = buff.readLine();
        if (inLine.equals("Cert pls")) {
            System.out.println(Arrays.toString(serverCert.getEncoded()));
            System.out.println(Arrays.toString(serverCert.getEncoded()).length());
            System.out.println(serverCert.getEncoded().length);
